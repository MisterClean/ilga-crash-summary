...
# Query OSM for Archer Avenue
archer_streets <- opq(bbox = CHICAGO_BBOX) %>%
...
# Create buffer around Archer Avenue and identify intersecting crashes
buffer <- st_buffer(archer_streets, dist = BUFFER_DISTANCE_M)
intersects <- st_intersects(crashes_with_district, buffer, sparse = FALSE)

# Filter crashes within buffer
crashes_with_district$is_archer_crash <- apply(intersects, 1, any)
archer_crashes <- crashes_with_district %>%
  filter(is_archer_crash)

# Calculate economic damages
archer_crashes <- calculate_economic_damage(archer_crashes)

# Create district summaries for Archer Avenue crashes
archer_senate_summary <- create_district_summary(archer_crashes, "senate_district")
archer_house_summary <- create_district_summary(archer_crashes, "house_district")

# Export summaries
export_summary(archer_senate_summary, "archer_senate_summary.csv")
export_summary(archer_house_summary, "archer_house_summary.csv")

# Create overall Archer summary
archer_summary <- archer_crashes %>%
  summarize(
...
export_summary(archer_summary, "archer_summary.csv")

# Create fatality subset
archer_fatalities <- archer_crashes %>%
  filter(fatality_count > 0)

# Transform back to WGS84 for mapping
archer_fatalities <- st_transform(archer_fatalities, 4326)
archer_streets <- st_transform(archer_streets, 4326)
buffer <- st_transform(buffer, 4326)

# Create Archer Avenue fatality map
archer_fatality_map <- leaflet() %>%
  ...
  setView(lng = -87.6847, lat = 41.8475, zoom = 12)

# Export map
export_map(archer_fatality_map, "archer_fatalities_map.html")

# Create heatmap of all crashes
archer_heatmap <- leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(
    data = buffer,
    fillColor = "blue",
  ...
  addHeatmap(
    data = st_transform(archer_crashes, 4326),
    lng = ~st_coordinates(geometry)[, 1],
    lat = ~st_coordinates(geometry)[, 2],
    intensity = ~1,
    blur = 20,
...
# Export heatmap
export_map(archer_heatmap, "archer_crashes_heatmap.html")

# Print summary statistics
cat("\nArcher Avenue Analysis Complete\n")
cat("Total Crashes:", sum(archer_crashes$crash_count, na.rm = TRUE), "\n")
cat("Total Fatalities:", sum(archer_crashes$fatality_count, na.rm = TRUE), "\n")
cat("Total Economic Damage: $", format(sum(archer_crashes$estimated_economic_damages, na.rm = TRUE), big.mark = ","), "\n")
