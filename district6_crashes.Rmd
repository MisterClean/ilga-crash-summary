...
# 1) Load senate district shapefile and filter for lakefront districts
...
# 2) Spatial join crashes with lakefront districts
lakefront_crashes <- sf::st_join(combined_crashes_sf, lakefront_districts, left = FALSE)

# 3) Transform to meter-based CRS (EPSG 3857) for accurate distance calculations
lakefront_crashes <- sf::st_transform(lakefront_crashes, 3857)

# 4) Keep District 6 crashes for district-specific analysis
crashes_with_district <- lakefront_crashes %>%
  filter(senate_district == "6")

# 5) Define a bounding box for Chicago and the buffer distance (150 ft ~ 30.48 m)
CHICAGO_BBOX <- c(-87.9401, 41.6445, -87.5239, 42.0230)
BUFFER_DISTANCE_M <- 45.72

# 6) Query OSM for the main US 41 portion of Lake Shore Drive
...
# 7) Query OSM for the northern extension by name
lsd_north_query <- opq(bbox = CHICAGO_BBOX) %>%
  add_osm_feature(key = 'highway') %>%
  # If the exact name in OSM is "North Jean Baptiste Point DuSable Lake Shore Drive",
  # set value_exact=TRUE. If not certain, leave it FALSE:
...
# 8) Extract just the geometry from each to avoid mismatched columns
us41_geom  <- sf::st_geometry(lsd_us41_lines)
north_geom <- sf::st_geometry(lsd_north_lines)

# 9) Combine the geometry into a single sf object
...
lake_shore_streets <- sf::st_sf(
  ...
  crs = sf::st_crs(lsd_us41_lines)  # same CRS as the original OSM lines
)

# 10) Transform to EPSG 3857 (to match lakefront_crashes) for buffering
...
# 2) Filter crashes within buffer and calculate counts for all lakefront districts
...
# 4) Create district-specific subsets for the time series visualizations
